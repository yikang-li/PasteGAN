...
# Input data
parser.add_argument(
    '--splits_json', default=os.path.join(VG_DIR, 'vg_splits.json'))
...
parser.add_argument('--output_mem_json',
                    default=os.path.join(VG_DIR, 'mem_bank.json'))
...
def main(args):
    ...
    object_id_to_obj = filter_objects(
        args, objects, obj_aliases, vocab, splits)
    ...
    for split_name, split_arrays in numpy_arrays.items():
        ...
        # remove h5 file if exist
        if os.path.exists(h5_path):
            print("Existing %s.h5 detected, deleting it..." % split_name)
            os.remove(h5_path)
    ...
    # added for creating memory bank
    print('Writing memory bank to "%s"' % args.output_mem_json)
    # number of names of objects
    key = 'object_idx_to_name'
    num_obj_name = len(vocab[key])
    print("there are in total", num_obj_name, "classes (names) of objects")
    # load train.h5
    train_file = os.path.join(args.output_h5_dir, "train.h5")
    f = h5py.File(train_file, 'r')
    object_names = np.array(f['object_names'])
    # initialize memory bank
    mem_bank = {}
    for i in range(num_obj_name):
        mem_bank[i] = []
    # append content inside
    for image_row_num, row in enumerate(object_names):
        for obj_id_in_img, name in enumerate(row):
            if name == -1:
                continue
            mem_bank[name].append((image_row_num, obj_id_in_img))
    with open(args.output_mem_json, 'w') as f:
        json.dump(mem_bank, f)
    # end of memory bank creation
...
def filter_objects(args, objects, aliases, vocab, splits):
        ...
        for obj in image['objects']:
            ...
            for name in obj['names']:
                ...
                if name in object_name_to_idx:
                    ...
                    break
            w, h = obj['w'], obj['h']
            too_small = (w < args.min_object_size) or (
                h < args.min_object_size)
            if too_small:
                num_too_small += 1
    ...
    print('Skipped %d objects with size < %d' %
          (num_too_small, args.min_object_size))
            ...
            for object_id in image_object_ids:
                ...
                if attributes is None:
                    object_attributes.append(
                        [-1] * args.max_attributes_per_image)
                    ...
                    pad_len = args.max_attributes_per_image - \
                        len(attribute_ids)
